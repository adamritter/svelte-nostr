...
export function getEventsByFilters(
  ...
  return filtered.toArray().then((events) => {
    let flat_events = events
      .map((event) => event.events || event.event || [])
      .flat();
    // @ts-ignore
    let query_infos: (IEvent & {query_info: QueryInfo})[] = events.filter(
      (event) => event.query_info
    );
