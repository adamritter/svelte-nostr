...
test("authors with times", () => {
    ...
    let result = getFiltersToRequest("sub", [filter], {}, events, [
        {filter: '{"authors":["pub1"]}', query_info: {req_sent_at: 5, received_events: 1}},
        {filter: '{"authors":["pub2"]}', query_info: {req_sent_at: 5, received_events: 1}},
    ]);
    expect(result).toEqual([{authors: ["pub1", "pub2"], limit: 133, since: 6}, {authors: ["pub3"], limit: 67}]);
});
...
test("#ps with times", () => {
    ...
    let events:Event[] = [
        {id: "id1", pubkey: "pub1", tags: [["#p", "p1"], ["#e", "e1"]], kind:0,
            created_at: 1, content: "HW"},
        {id: "id2", pubkey: "pub2", tags: [["#p", "p2"], ["#e", "e1"]], kind:0,
            created_at: 2, content: "HW"},
        {id: "id3", pubkey: "pub2", tags: [["#p", "p1"], ["#e", "e1"]], kind:0,
            created_at: 1, content: "HW"},
        ];
    ...
    expect(result).toEqual([{"#p": ["p1", "p2"], limit: 133, since: 6}, {"#p": ["p3"], limit: 67}]);
});
...
test("Multiple filters with similar times", () => {
    ...
    expect(result).toEqual([{authors: ["pub1", "pub2"], since: 2}, {authors: ["pub3"]}, {authors: ["pub1", "pub2", "pub3"]}]);
});
...
test("Multiple filters with different times", () => {
    ...
    let events:Event[] = [
        {id: "id1", pubkey: "pub1", tags: [["#p", "p1"], ["#e", "e1"]], kind:0,
            created_at: 3600, content: "HW"},
        {id: "id2", pubkey: "pub2", tags: [["#p", "p1"], ["#e", "e1"]], kind:0,
            created_at: 7200, content: "HW"},
        {id: "id3", pubkey: "pub2", tags: [["#p", "p1"], ["#e", "e1"]], kind:0,
            created_at: 3600, content: "HW"},
        ];
    let result = getFiltersToRequest("sub", [filter1, filter2], {}, events, []);
    expect(result).toEqual([{authors: ["pub1"], since: 3601}, {authors: ["pub2"], since: 7201}, {authors: ["pub3"]}, {authors: ["pub1", "pub2", "pub3"]}]);
});
