<script>
  import { relayPool } from "./../lib/subscription.ts";
  ...
  import {
    publishedStore,
    receivedStore,
    publishedProfilesStore,
    publishedProfilesByPubKeyStore,
    contactsStore,
    eventsFromFollowedStore,
    subscribeAndCacheResultsStore,
    repliesFilter,
    repliesStore,
    getEvents,
    groupByTag,
    stringifyUnique,
    filtersFromEventTags,
    suggestions,
  } from "$lib/svelte-nostr-stores";
  import { Kind } from "nostr-tools";
  import { groupBy, mapBy } from "$lib/collection-helpers";
  import {
    mergeSimilarFilters,
    requestFilter,
  } from "$lib/get-filters-to-request";
  ...
  $: received = $events.filter(
    (e) => e.tags.filter((t) => t[0] == "p" && t[1] == pubKey).length > 0
  );
  ...
  $: publishedProfilesByPubKey = mapBy(
    published_profiles,
    (item) => item.pubkey
  );
  ...
  $: pageEventsFinal = pageInfo.viewauthor
    ? $events.filter((event) => event.pubkey == pageInfo.viewauthor)
    : $events.filter(
        (event) => event.pubkey == pubKey || followed.includes(event.pubkey)
      );
  let rendered = false;
  ...
  <section>
    <a
      ...
      on:click={() => {
        startTime = Date.now();
        rendered = false;
        pageInfo = { page: "posts", viewauthor: null };
      }}><b>Posts</b></a
    >
    <a
      href=""
      on:click={() => {
        startTime = Date.now();
        rendered = false;
        pageInfo = { page: "posts_replies", viewauthor: null };
      }}><b>Posts and replies</b></a
    >
    ...
    <span>
      <Photo
        data={publishedProfilesByPubKey.get(pubKey)}
        on:click={() => {
          rendered = false;
          pageInfo = { viewauthor: pubKey, page: "posts_replies" };
        }}
      />
    ...
    <span style="display: flex; gap: 20px; flex-direction: row">
      <span style:display={pageInfo.page == "posts" ? "block" : "none"}>
        <PublicNotes
          ...
          on:viewauthor={(e) => {
            rendered = false;
            pageInfo = { viewauthor: e.detail, page: "posts_replies" };
          }}
        /></span
      >
      ...
      {#if data.private_key}
        <!-- TODO: use window.nostr encode / decode instead if the private key doesn't exist -->
        <span style:display={pageInfo.page == "dms" ? "block" : "none"}
          ><DMs
            {events}
